...
func newServer() *RpcServer {
	...
	opts = append(opts, ServerUnknown())
	...
	// opts = append(opts, grpc.EnableTracing)
...
func shouldSkip(method string) bool {
	...
	for _, i := range skipRouters {
		if strings.HasSuffix(method, i) {
			grpclog.Info("Skipping route", method)
			return true
		}
	}
...
func ServerUnknown() grpc.ServerOption {
	return grpc.UnknownServiceHandler(
		func(srv interface{}, stream grpc.ServerStream) error {
			fmt.Printf("ServerUnknown srv: %+v\n", srv)
			return nil
		},
	)
}
